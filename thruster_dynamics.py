...
def get_thruster_com_and_orientations(device, cfg):
  """
  ...
  thruster_info = [None for i in range(len(cfg.items()))]
  for name, info in cfg.items():
    if len(info["rot"]) == 3:
      pose = (info["position"][0], info["position"][1], info["position"][2], info["rot"][0], info["rot"][1], info["rot"][2])
      thruster_info[info["id"]] = create_tf_rpy(*pose)
    elif len(info["rot"]) == 4:
      pose = (info["position"][0], info["position"][1], info["position"][2], info["rot"][0], info["rot"][1], info["rot"][2], info["rot"][3])
      thruster_info[info["id"]] = create_tf_quat(*pose)
